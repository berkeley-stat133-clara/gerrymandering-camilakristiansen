...
```{r}
votes <- votes |>
  mutate(dem_prop = d_votes / (d_votes + r_votes))

mean_median <- mean(votes$dem_prop) - median(votes$dem_prop)
mean_median
```
...
```{r}
gap_calc <- with(votes, {
  total_ballots <- d_votes + r_votes
  half_point <- total_ballots / 2
  wasted_dem <- ifelse(d_votes > r_votes, d_votes - half_point, d_votes)
  wasted_rep <- ifelse(r_votes > d_votes, r_votes - half_point, r_votes)
  sum(wasted_rep - wasted_dem) / sum(total_ballots)
})
gap_calc
```